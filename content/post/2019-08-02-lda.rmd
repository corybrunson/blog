---
draft: true
title: "struggling to discriminate"
author: ~
date: 2019-08-02
categories: [methodology]
tags: [ordination,biplot,linear discriminant analysis]
---
...
For over a year (intermittent, not cumulative) i've been developing an R package, [ordr](https://github.com/corybrunson/ordr), for incorporating ordination techniques into a tidyverse workflow. (Credit where it's due: Emily Paul, a summer intern from Avon High School, helped extend the package functionality and has since used it in other projects and provided critical feedback on its design.) This week, i finally deployed [a pkgdown website](https://corybrunson.github.io/ordr/) and tweeted a meek solicitation for feedback.[^feedback] Interestingly, the one component that held be back, that i put off time after time before finally grinding out a solution over the past few weeks, was the component that originally complemented PCA in [Vince Vu's original inspiring ggbiplot package](https://github.com/vqv/ggbiplot): The MASS package implementation of linear discriminant analysis.

[^feedback]: If you use ordination and ggplot2, not necessarily together (yet), i'd be grateful for your feedback, too!

A big part of the reason is that [the source code of `MASS::lda()`](https://github.com/cran/MASS/blob/master/R/lda.R) looks nothing like the tidy definition of LDA found in textbooks. (I have on my shelf my mom's copies of [Tatsuoka's _Multivariate Analysis_](https://books.google.com/books/about/Multivariate_Analysis.html?id=HHNnBDaNsuUC) and of [Nunnally's _Psychometric Theory_](https://books.google.com/books/about/Psychometric_theory.html?id=WE59AAAAMAAJ), but there are several similar introductions online, for example [Raschka's](https://sebastianraschka.com/Articles/2014_python_lda.html).) In the standard presentation, LDA amounts to an eigendecomposition of the quotient ${C_W}^{-1}C_B$ of the between- by the within-group covariance matrices. (This is equivalent, up to a scale factor, to the eigendecomposition of the same quotient of the corresponding scatter matrices $S_\bullet=nC_\bullet$, assuming the covariances were calculated from a population rather than a sample perspective.)
But `MASS::lda()` not only makes no use of `eigen()`; presumably for the sake of efficiency, it relies on an alternating sequence of redefinitions of a transformed data set and a scaling matrix, and the resulting braid is difficult to follow by reading the (minimally documented) code.
...
The code will be executed in sequence, punctuated by printouts of key objects defined along the way. This requires assigning values to the function arguments. I'll take as my example the famous diabetes data originally analyzed by [Reaven and Miller (1979)](https://link.springer.com/article/10.1007/BF00423145), available from the heplots package, which consists of a BMI-type measure and the results of four glucose and insulin tests for 145 patients, along with their diagnostic grouping as non-diabetic ("normal"), subclinically ("chemical") diabetic, and overtly diabetic. For our purposes, the only other required parameter is `tol`, the tolerance at which small values encountered along the matrix algebra are interpreted as zero.
...
As a recurring illustration, the code chunk below plots steady state plasma glucose against test plasma glucose (the fifth versus the third column) across the patients in the sample, using a fixed aspect ratio.
...
The first several lines of `lda.default()` capture summary information about the data table `x` (and ensure that it is a matrix) and the group assignments `grouping`: `n` is the number of cases in `x`, `p` is the number of variables, `g` is `grouping` coerced to a factor vector, `lev` is a vector of groups, `counts` is a vector of the number of cases assigned to each group, `proportions` is a vector of their proportions of `n`, and `ng` is the number of groups.

It will help to keep track of some mathematical notation for these concepts in parallel: Call the data matrix $X\in\mathbb{R}^{n\times p}$, the column vector of group assignments $g\in\mathbb{R}^{n}$, and a diagonal matrix $N\in\mathbb{N}^{q\times q}$ of group counts $n_1,\ldots,n_q$. Note that `lev` and `ng` correspond to $c$ and $q$, respectively. Additionally let $G=(\delta_{g_i,k})\in\{0,1\}^{n\times q}$ denote the 0,1-matrix with $i,k$-th entry $1$ when case $i$ is assigned to group $k$. (I'll use the indices $1\leq i\leq n$ for the cases, $1\leq j\leq p$ for the variables, and $1\leq k\leq q$ for the groups.)
...
The steps are straightforward, with the exception of the handling of prior probabilities (`prior`), which is beyond our scope and which i've excised from the code. (When not provided, `prior` takes the default value `proportions`, which are explicitly defined within `lda.default()`.) If `grouping` is a factor that is missing elements of any of its levels, then these elements are removed from the analysis with a warning. I'll also skip the lines relevant only to cross-validation (`CV = TRUE`), which include a compatibility check here and a large conditional chunk later on.
...
The conditional statement requires that at least some variances be nonzero, up to the tolerance threshold `tol`. Finally, `scaling` is initialized as a diagonal matrix $S_0$ of inverted variable standard deviations $\frac{1}{\sigma_j}$, or of the square roots of the variable precisions.
...
I'll refer to ${X_0}$ as the "within-group centered data". The group centroids are returned as the named member `means` of the 'lda'-class object returned by `lda.default()`. The standard deviations of the five variables, stored in `f1`, are evident in their pairwise scatterplots:
...
The alternative calculation is $S_1=S_0\hat{V}{\hat{\Lambda}}^{-1/2}$, where $\hat{C}={\hat{V}}^\top\hat{\Lambda}\hat{V}$ is the eigendecomposition of the covariance matrix of $\hat{X}$ so that $\hat{V}$ is the same and $\hat{\Lambda}={\hat{D}}^{1/2}$. (${\hat{C}}^{-1/2}=\hat{V}{\hat{\Lambda}}^{-1/2}{\hat{V}}^\top$ is the symmetric square root of $\hat{C}$.)
...
While `rank` is 5 at this stage, indicating that variation is detected in each of the variables, the final computational step will determine the rank of the LDA itself in terms of the transformed group centroids.
...
`lda.default()` relies on an equivalent calculation $C_B=\frac{1}{q}\overline{Y}^\top\overline{Y}$, using $\overline{Y}=N(\overline{X}-\mathbb{1}_{q\times 1}\overline{x})$, which explicitly weights the centered group centroids and produces the same scatter matrix as $Y$.
Under the transformed coordinates ${X_0}S_1$, the matrix quotient to be eigendecomposed becomes $$({S_1}^\top C_WS_1)^{-1}({S_1}^\top C_BS_1)=I_p{S_1}^\top C_BS_1=\frac{1}{q}(YS_1)^\top(YS_1)=\frac{1}{q}(\overline{Y}S_1)^\top(\overline{Y}S_1),$$
