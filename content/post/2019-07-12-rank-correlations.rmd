...
Meanwhile, the geometry of the variables is usually understood through their covariances or correlations. The covariance between two variables $i,j$ is proportional to their inner product $$\textstyle \operatorname{cov}_{ij}=\frac{1}{n}v_i\cdot v_j=\frac{1}{n}\lVert v_i\rVert\lVert v_j\rVert\cos\theta_{ij}\text,$$ so that the cosine the angle $\theta_{ij}$ between them equals their correlation:
$$\cos\theta_{ij}=\frac{\operatorname{cov}_{ij}}{\sqrt{\operatorname{cov}_{ii}\operatorname{cov}_{jj}}/n}=\frac{\operatorname{cov}_{ij}}{\sigma_i\sigma_j}=r_{ij}$$
