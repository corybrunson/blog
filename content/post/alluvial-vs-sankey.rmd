...
2. Sankey diagrams:
    ...
    * [Sankey diagram human circulatory system esp](https://commons.wikimedia.org/wiki/File:Sankey_diagram_human_circulatory_system_esp.svg.png), Wikimedia Commons
...
3. Parallel sets plots (that are _not_ alluvial plots):
    ...
    * [Stacked area alluvial diagram](https://xeno.graphics/stacked-area-alluvial-diagram/), Xenographics
    ...
    * [Sankey diagram on global blue virtual water flows](https://www.researchgate.net/figure/Figure-S2-Sankey-diagram-on-global-blue-virtual-water-flows-Sankey-diagram-for-virtual_fig6_303306020), Serrano et al
4. Alluvial plots:
    ...
    * [Alluvial diagram for mapping change in Japanese Topsy network](https://www.researchgate.net/figure/Alluvial-diagram-for-mapping-change-in-Japanese-Topsy-network-Contents-of-surviving_fig2_267734552), Lu and Brelsford
...
Indeed, i haven't seen a discussion anywhere else of whether a parallel sets or alluvial plot puts repeated categories in the same order, or whether the parallel sets are stacked so that the perpendicular axis measures their cumulative size, or whether either of these features is relevant.
...
That's still not to say i was first: Hofmann and Vendettouli wrote ggparallel to stack the sets in each dimension and retain a vertical axis in their plots. There may well be other such implementations, but i'm only familiar with those in R.

### Utility

My argument has two steps. First, i claim that the features that distinguish alluvial from parallel sets plots---consistent ordering of sets and a cumulative weight axis---have practical importance.
In particular, they have importance _beyond_ the ability to visually distinguish the sets and compare the total weight along each dimension, which is a feature of parallel sets plots generally.
Consider the following examples, most obtained by searching for "ggalluvial" in Google Scholar; each is coupled with some more accessible examples tweetet by users:

**Repeated categorical measures data:**
Several studies have used alluvial plots to represent data consisting of partitions of cases into the same (or overlapping) classification schemes at different times, in particular before and after some intervention or other significant event.
See [Baudron, Ndoli, Habarurema, and Silva (2019)](https://www.sciencedirect.com/science/article/pii/S0378429018317337), [Kissinger and Reznik (2019)](https://www.sciencedirect.com/science/article/pii/S0195925518302026), [Muenchow, Schäfer, and Krüger (2019)](https://onlinelibrary.wiley.com/doi/full/10.1111/gec3.12441), [Chong et al (2019)](https://www.jneurosci.org/content/39/28/5534.abstract), and [Schlotter et al (2019)](https://onlinelibrary.wiley.com/doi/full/10.1002/ejhf.1547) for examples.
...
* [Repeat offender's next crime more likely to be lower degree](https://twitter.com/KenSteif/status/1006542071375761408) (@KenSteif)
* [Mobility Among Nobel Laureates in the Sciences](https://twitter.com/frau_dr_barber/status/1130167116164927488) (@frau_dr_barber)
* [What children learned about their HIV status over time](https://twitter.com/ericpgreen/status/1133840554968666112) (@ericpgreen)
* [EU Vote to Next General Election Vote](https://twitter.com/5amStats/status/1135153961227509762) (@5amStats; starboard plot)

**Multipartite network data:**
A handful of studies have used alluvial plots to represent multipartite graphs, which necessarily satisfy the constraint that the total degree of the nodes in each part is the same. In particular, genomic analyses produce one-to-one connections among stages in transcription processes (lncRNA, miRNA, and mRNA), which have been encoded into alluvial plots by [Zheng et al (2018)](https://peerj.com/articles/6091/), [Long et al (2019a)](https://cancerci.biomedcentral.com/articles/10.1186/s12935-019-0817-y), and [Long et al (2019b)](https://www.frontiersin.org/articles/10.3389/fonc.2019.00649/full). (See also [Vazquez Bernat et al (2019)](https://www.frontiersin.org/articles/10.3389/fimmu.2019.00660/full).) While these authors excluded the cumulative weight axis, through the fixed heights of the stacked sets the plots communicate that the total degree at each stage is the same.

Some examples from Twitter:

* [GPU Designers & Manufacturers](https://twitter.com/MyriamCTraub/status/1169236685160402946) (@MyriamCTraub)
* [Train trips departing from Paris stations in July 2017](https://twitter.com/BenMoretti/status/1100378930865827840) (@BenMoretti)

**Censored data:**
Finally, something i think alluvial plots do exceptionally better than general parallel sets plots is accentuate censoredness in data. Check out [Seekatz et al (2018)](https://www.sciencedirect.com/science/article/pii/S1075996418301021) and [Sjoding, Gong, Hass, and Iwashyna (2019)](https://journals.lww.com/ccmjournal/Fulltext/2019/01000/Evaluating_Delivery_of_Low_Tidal_Volume.8.aspx), who use alluvial plots to depict changes in subjects' status across several time points with a specific set (or blank space where it would be) for subjects who became unavailable later in the study. The cumulative weight axis and gridlines allow the reader to immediately discern the reduction in sample size at each step.
...
* [My Data Science Job Hunt](https://twitter.com/mdneuzerling/status/1170826000122273792) (@mdneuzerling)

That's really the entirety of my argument for the distinction; my second step is merely a finishing fluorish.
